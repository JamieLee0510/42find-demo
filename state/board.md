...
> ## 🚢 PR #6 合并，然后自己一跑就出事（2026-09-07 · `4f2ba43`）
> 30 个提交、47 个文件一次进 main，CI 第一次跑就绿。issue #4 自动关了；
> **#5 还开着**——PR 标题写 `closes #4 #5`，GitHub 只认第一个，要写成 `closes #4, closes #5`。
>
> **核心承诺在真实语料上兑现了**（45,624 字节自然中文，`demo-20260904/*.md`）：
> 搜繁体「檢索」在简体文档里命中 **10 处**，`rg` 是 **0 处**。这是这个工具存在的理由。
>
> **但教材第五节说的事照样发生了——合并完自己一跑，出了三个问题：**
> ① **输出只给命中的词，不给那一行**。`.../01-research.md:1:判据` vs rg 的
>    `1:# 01 · 调研：判据先行，结论定位到行`。搜到了却看不到上下文，得再打开文件。
>    ⚠️ **十三个评审视角（六轮双谱系 + 四视角 simplify + pr-ready 三视角）没有一个发现它**
>    ——它们审的是「代码对不对」，不是「这东西能不能用」。
> ② 同一行两处命中，不带 `--column` 时输出两行完全一样，看着像重复。
> ③ 已知的 `| head` panic 退 101，真实使用第一次就撞上（玩具语料藏得住，真语料藏不住）。
>
> **另外两条数字要更正**：语料是 **1462 字节**不是 871（`board.md:131` 在 `1621e14` 扩容后没更新，
> 我照抄了那个旧数好几次）；**精确率 100% 和召回率一样有自证成分**——语料按查询集反向构造，
> 几乎没有能产生假阳性的自然噪声。此前只自陈了召回那一半。
>
> **下一步**：关 #5（先回复再改状态）· 把 ①②③ 开成 issue · `vault/raw/` 放真语料 · 课后跑体检。

> ## 🔧 issue #7 · 输出层：给整行、断管道不 panic、写失败不静默（2026-09-07 · `fix/issue-7-output-layer`）
> 「合并完自己一跑」出的三条全部收掉，走的是 `/dev-launch` 高确定性快走（根因代码级确诊，没占人时间拍板）。
>
> **改了什么**
> ① `Match` 从 `{line, col, text: String}` 变成 `{line, col, len, line_text: &'a str}`——
>    借用被扫文本、**整个结构只留一个字符串视图**，命中那一段由 `text()` 现算。
>    留两个重叠视图正是 issue #7 第一症状的形状（**输出层挑错了字段**），类型上消掉它。
> ② 输出对齐 `rg`：默认 `路径:行号:整行` 且一个匹配行只出一行；`--column` 保持**逐命中**
>    （`bench.sh` 走的就是它，34 条黄金查询集的分母不能动）。
> ③ `println!`/`eprintln!`/`print!` 全部换掉。命中走 `BufWriter<StdoutLock>`（64 KiB）+ `writeln!`；
>    诊断走忽略写失败的 `warn!` 宏。「哪些写失败算错」收成唯一判据 `is_fatal_write_err`，
>    告警侧与退出码侧共用——第一版它住了两处，被三条清理视角同时点名。
>
> **两个数（同条件 A/B，改前二进制由 `git archive origin/main` 现建）**
> - 固定 44 KB 语料：2.2 ms vs 2.3 ms，比值 **1.04 ± 0.49——测不出差别**。
>   ⚠️ 先前单跑看到的 1.7→2.2 是机器状态，不是改动；这个语料上延迟本来就量不出输出层。
> - 11 MB / 232,000 行 / 112,000 处命中：105.9 ms → **49.7 ms（2.13× ± 0.07）**，
>   而且输出的还是更长的整行。主因是每处命中省掉一次 malloc。
> - **契约证据**：改前改后 `--column` 输出的 `file:line:col` 三列（`bench.sh` 真正取的那三列）
>   在 5 个查询、上千处命中上**逐字节相同**。三个数实测未退：rg 50%（58/115）/ 100%；
>   42find 99%（114/115）/ 100%。
>
> **闸门多长了一副牙齿**：`clippy::print_stdout` / `print_stderr = "deny"`（restriction 组，不在 `all` 里）。
> 「不许再用 print 宏」从注释升成**编译期规则**，跑在既有 `cargo clippy` 那道闸里。
> 反向验过：插一行 `println!` → clippy rc=101。
>
> **一条实测更正**：issue 里「输出到坏 fd 退 2」这个**具体例子在 Rust 里到不了我们的代码**——
> std 对 stdout 的 EBADF 是**故意吞掉**的（最小程序复现：裸 `write(1,..)` 返 -1，
> 而 `Stdout::write_all` + `flush` 返 `Ok`；`rg` 同样行为）。端到端改用 **EIO**（pty 从属端关闭）验：
> **rc=2 并报 stderr**，同条件下 `println!` 版退 101。判据本身由单测的真值表守。
>
> **我自己犯的一个错**：`git add -A src/` 把**别的会话在途的** `src/standard.md` ·
> `src/negatives.md` · `src/check.sh` 扫进了提交（两条清理视角同时发现）。已 `git rm --cached`
> 摘出、文件原样留在盘上未动。**下次提交只点名自己改的文件，不用 `-A` 扫目录。**
>
> **教训已沉淀**：`state/memory/20260907-真实使用是一层独立验证.md`——
> 十三个评审视角全绿，合并完自己一跑出三个问题；玩具语料会整类藏住与规模相关的失败。

> ## 🔁 issue #7 的评审链：十九个视角，每一层都还在抓东西（2026-09-07）
> 一条 issue 走完 `/dev-launch` 全流程，**四层评审各自抓到了别人没抓到的**——
> 这本身就是「双谱系不可降级为单评审」那条纪律的又一次实证：
>
> | 层 | 视角数 | 抓到 |
> |---|---|---|
> | `/simplify` | 4 | `Match` 上两个重叠字符串视图（3 视角同时点名）· 写失败判据住两处（3 视角）· print 宏禁令还是人判的（2 视角）· `display()` 每行重做 · `BufWriter` 容量 |
> | `pr-ready` | 3 | **1×P0**（`--column` 输出对行长呈平方）· NUL/OSC 注入面 · `Match` 字段全 `pub` 致 `text()` 可 panic · BOM · TTY 不流式 · `bench.sh` 的静默假阴性 |
> | 双谱系①（GPT/DeepSeek） | 2 | **1×P1**（`str::lines()` 吃掉 `\r`）· **1×P2**（输出限住了内存没限住）· 5 处 HELP 与代码不符 |
> | 双谱系②（GPT/DeepSeek） | 2 | 截断标注报的是上限、不是实际打出去的字节数（回退到字符边界后差 2）· `pipe.rs` 缺 unix 门 |
>
> **四个最值钱的发现，issue 原文一个都没写**：
> ① `--column` 输出对行长**平方**放大：10 KB 单行 → 106 MB，2.7 MB minified JSON → 333 GB。
>    **这是我自己改出来的**——为了让带列号时也看得到上下文。收成默认 512 字节上限 +
>    `--max-columns 0` 逃生口。注意**只是把平方降成线性**，不是降成常数。
> ② **内存没限住**：191 MB 单行含 NUL 的文件，峰值 **19.5 GB**（只为打印一句「二进制文件有命中」）。
>    `search`/`search_line` 改惰性迭代器 + 零分配 `has_match` → **201 MB**，只剩文件本身。
> ③ `str::lines()` 吃掉行尾 `\r`：CRLF 文件吐出来的「整行」不是原文的字节，
>    同时打脸本仓两句白纸黑字——HELP 的「与 rg 同形」、`search` 的「不改语料」。**剥掉一个字节也是改。**
> ④ **黄金查询集里有一条一直在假测**：`bench.sh` 的 `eval` 会把已展开的 `"$q"` 再切一次词，
>    `the quick` 变成「查 the，在路径 quick 和 . 里找」→ 引擎退 2 → 被行尾的
>    `2>/dev/null || true` 抹平成「0 处命中」。**34 条里的 1 条，两轮评审都没发现。**
>    ——这正是二进制里刚消灭的静默假阴性，在 harness 外面一层原样复发。
>
> **闸门又长了三颗牙**：`clippy::print_stdout` / `print_stderr` / `dbg_macro = "deny"`。
> 「不许用 print 宏」从注释升成**编译期规则**，跑在既有 `cargo clippy` 那道闸里。
> 反向验过：插一行 `println!` → clippy rc=101。
>
> **我自己犯的两个错，都记下来**：
> ① `git add -A src/` 把**别的会话在途的** `src/standard.md`·`negatives.md`·`check.sh`
>    扫进了提交（两条清理视角同时发现）。已 `git rm --cached` 摘出，文件原样留在盘上没动。
>    **下次提交只点名自己改的文件，不用 `-A` 扫目录。**
> ② 上一个提交刚写完「留两个重叠视图拦不住下一个人挑错字段」，转头就留了一堆 `pub` 可写字段，
>    让 `Match::text()` 三种构造下都能 panic。**同一个道理，一轮之内犯了两次。**
>
> **一条实测更正**：issue 原文举的「输出到坏 fd 退 2」在 Rust 里**到不了我们的代码**——
> std 对 stdout 的 EBADF 是故意吞掉的（最小程序复现：裸 `write(1,..)` 返 -1，
> 而 `Stdout::write_all` + `flush` 返 `Ok`；`rg` 同样行为）。端到端改用 EIO（pty 从属端关闭）验：
> rc=2 并报 stderr，同条件下改前的 `println!` 版退 101。
>
> **双谱系第二轮：两条谱系都判无 P0/P1/P2，收敛。** DeepSeek 系明写「这轮 diff 干净，可合」。
> 第二轮各抓到的都是 P3，已收三条：截断标注报的是**上限**而不是**实际打出去的字节数**
> （回退到字符边界后差 2）· `tests/pipe.rs` 缺 `#![cfg(unix)]` 门 · `stdout()` 被调两次。
>
> **两条 P3 明确不做，理由记下来**：
> ① GPT 系建议给 core 加「逐匹配行首个命中」的 `search_lines`，让默认模式不必扫完整行。
>    **不做**：那会把「默认模式去重」这个判据拆到两处——`search_all` 挑哪个 API、`emit` 看
>    `column` 旗标，两处必须同步，传错组合就静默出错。现在的哨兵版**判据只在 `emit` 一处**。
>    本仓吃过三次「一个判据住两处」的亏，省那点扫描不值当。省下的也只是命中后剩余的
>    `match_at` 调用，扫描本身省不掉。
> ② GPT 系指出截断取行首，命中在 512 字节之后就看不到——**实测 `rg -M` 与
>    `--max-columns-preview` 同样如此**（`rg` 甚至整行换成「Omitted long matching line」）。
>    我们是**对齐的**，不是更差。写进 HELP，不改行为。
>
> **`find42-core` 是源码破坏性变更**（`Match` 字段私有化 + `search` 返回迭代器）。
> version 仍是 `0.0.0`、未发布、仓内外无消费者，记在 PR 说明里即可。
>
> **当前**：37 个测试（16 core + 13 CLI + 8 端到端真起进程）· 三道闸门各自 rc=0 ·
> 三个数与基线逐位相同（rg 50% 58/115 / 100%；42find 99% 114/115 / 100%）。
> **§7 收敛完成 → PR #8 已开**（`fix/issue-7-output-layer` → `main`，5 个提交，`closes #7`）。
> **不自动合并**——等人工合并。本地 `main` 上那个未推的 `1a6fc01` 也一并推了，PR 因此只含本 issue 的 5 个提交。
>
> **两条既有问题按范围划出去，已开 follow-up**：
> - issue #9 `read_to_string` 无上限（8 GB 稀疏文件峰值 8.6 GB，rg 2.5 MB；分配失败退 134 / OOM 退 137，
>   都在承诺的 {0,1,2} 之外）。⚠️ 动手前必读：`Match` 借的是整段文本，改流式读**必须与借用设计一起改**。
> - issue #10 TOCTOU（`stat` 与 `open` 之间可被换成 FIFO → 永久阻塞）。
>   这个仓为「永久阻塞」花过一整轮，防护建对了地方、建错了时刻。纯 std 收法：打开后用
>   `File::metadata()`（fstat，作用在 fd 上）再确认一次。
